...
function JobNode({ data }: NodeProps) {
  ...
  const acquiredCount = requiredSkills.filter(
    s => s && s.status !== 'locked'
  ).length;
  const total = job.requiredSkillIds.length;
  const isUnlocked = acquiredCount === total;
  const progress = total > 0 ? (acquiredCount / total) * 100 : 0;
      ...
      >
        ...
        <div className="mb-2">
          <div className="flex justify-between items-center mb-1">
            <span className="text-[10px] text-gray-500 uppercase tracking-wider">Skills</span>
